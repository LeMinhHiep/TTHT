...
namespace MVCData.Helpers
{
    public class SqlRoutines
    {
        ...
        private void UpdateWarehouseBalance()
        {
            ...
            queryString = queryString + "                               SELECT      ActionTable.HotelID, ActionTable.ServiceID, PurchaseInvoiceDetails.Quantity, PurchaseInvoiceDetails.Quantity AS PurchaseInvoiceQuantity, PurchaseInvoiceDetails.GrossAmount AS AmountCost" + "\r\n";
        ...
        private void WarehouseJournal()
        {
            ...
            queryString = queryString + "                                   ServiceID, HotelID, SUM(QuantityBegin) AS QuantityBegin, SUM(AmountBegin) AS AmountBegin, SUM(QuantityInput) AS QuantityInput, AVG(UnitPriceInput) AS UnitPriceInput, SUM(VATAmountInput) AS VATAmountInput, SUM(GrossAmountInput) AS GrossAmountInput, SUM(QuantityOutput) AS QuantityOutput, SUM(AmountOutput) AS AmountOutput " + "\r\n";
            ...
            queryString = queryString + "                                       WarehouseBalanceDetail.ServiceID, WarehouseBalanceDetail.HotelID, WarehouseBalanceDetail.Quantity AS QuantityBegin, CASE WHEN (@IsAmountIncluded = 1) THEN WarehouseBalanceDetail.AmountCost ELSE 0 END AS AmountBegin, 0 AS QuantityInput, 0 AS UnitPriceInput, 0 AS VATAmountInput, 0 AS GrossAmountInput, 0 AS QuantityOutput, 0 AS AmountOutput " + "\r\n";
            queryString = queryString + "                           FROM        WarehouseBalanceDetail " + "\r\n";
            queryString = queryString + "                           WHERE       WarehouseBalanceDate = @WarehouseBalanceDate " + SWI + "\r\n";
            ...
            queryString = queryString + "                                       PurchaseInvoiceDetails.ServiceID, PurchaseInvoices.HotelID, CASE WHEN PurchaseInvoices.PurchaseInvoiceDate < @FromDate THEN PurchaseInvoiceDetails.Quantity ELSE 0 END AS QuantityBegin, 0 AS AmountBegin, CASE WHEN PurchaseInvoices.PurchaseInvoiceDate >= @FromDate THEN PurchaseInvoiceDetails.Quantity ELSE 0 END AS QuantityInput, CASE WHEN PurchaseInvoices.PurchaseInvoiceDate >= @FromDate THEN PurchaseInvoiceDetails.UnitPrice ELSE 0 END AS UnitPriceInput, CASE WHEN PurchaseInvoices.PurchaseInvoiceDate >= @FromDate THEN PurchaseInvoiceDetails.VATAmount ELSE 0 END AS VATAmountInput, CASE WHEN PurchaseInvoices.PurchaseInvoiceDate >= @FromDate THEN PurchaseInvoiceDetails.GrossAmount ELSE 0 END AS GrossAmountInput, 0 AS QuantityOutput, 0 AS AmountOutput " + "\r\n";
            queryString = queryString + "                           FROM        PurchaseInvoices INNER JOIN " + "\r\n";
            ...
            queryString = queryString + "                                       BillingDetail.ServiceID, ListRoom.HotelID, CASE WHEN BillingMaster.DepartureDate < @FromDate THEN -BillingDetail.Quantity ELSE 0 END AS QuantityBegin, 0 AS AmountBegin, 0 AS QuantityInput, 0 AS UnitPriceInput, 0 AS VATAmountInput, 0 AS AmountGrossInput, CASE WHEN BillingMaster.DepartureDate >= @FromDate THEN BillingDetail.Quantity ELSE 0 END AS QuantityOutput, CASE WHEN BillingMaster.DepartureDate >= @FromDate AND (@IsAmountIncluded = 1) THEN ROUND(BillingDetail.Quantity * WarehouseBalancePrice.UnitPrice, 0) ELSE 0 END AS AmountOutput " + "\r\n";
            queryString = queryString + "                           FROM        BillingMaster INNER JOIN " + "\r\n";
            queryString = queryString + "                                       BillingDetail ON BillingMaster.BillingID = BillingDetail.BillingID INNER JOIN " + "\r\n";
